...
async def daily_stats(
    ...
    cache_key = f"stats:daily:{target_date.isoformat()}"
    ...
    total_employees = (
        db.query(func.count(User.id)).filter(User.role != RoleType.SUPER_ADMIN).scalar()
        or 0
    )
    ...
    stats = (
        ...
        .group_by(AttendanceLog.action_type)
        .all()
    )
    ...
    result = {
        ...
        "attendance_rate": round(present_count / total_employees * 100, 1)
        if total_employees > 0
        else 0,
    }

    redis_client.set_json(cache_key, result, settings.CACHE_STATS_TTL)
    return result
...
async def attendance_trend(
    days: int = Query(30, ge=1, le=90, description="统计天数（最大90天）"),
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user),
):
    """获取最近 N 天的考勤趋势"""
